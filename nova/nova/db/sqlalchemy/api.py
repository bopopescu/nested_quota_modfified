...
#@require_admin_context
def quota_allocated_update(context, project_id,child_list):
    allocated_quota={}
    for child_id in child_list:
        quota_limit=quota_get_all_by_project(context,child_id)
        del quota_limit["project_id"]
        for resource in quota_limit:
            try:
                allocated_quota[resource]+=quota_limit[resource]
            except KeyError:
                allocated_quota[resource]=quota_limit[resource]

    for resource,allocated in allocated_quota.items():
        quota_ref = models.QuotaUsage
        try:
            ...
            session =get_session()
            _quota_usage_create(context, project_id, None, resource, 0,
                        0,allocated,0,session)
...
def _quota_usage_get_all(context, project_id, user_id=None):
    ...
    for row in rows:
        if row.resource in result:
            ...
            result[row.resource]['allocated'] += row.allocated
        else:
...
def _quota_usage_create(context, project_id, user_id, resource, in_use,
                        reserved,allocated=0,until_refresh=0, session=None):
    ...
    quota_usage_ref.allocated = allocated
