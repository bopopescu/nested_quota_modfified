...
################
    def get_settable_quotas(self, context, resources, project_id,
        ...
        """
        settable_quotas = {}
        db_parent_proj_quotas = {}
        parent_project_quotas = {}   
        db_proj_quotas = db.quota_get_all_by_project(context, project_id)
        if parent_id:
            db_parent_proj_quotas = db.quota_get_all_by_project(context, parent_id)
            parent_project_quotas = self.get_project_quotas(context, resources,
                                                parent_id, remains=True,
                                                project_quotas=db_parent_proj_quotas)
        ...
        if user_id:
            ...
            for key, value in user_quotas.items():
                ...
                minimum = value['in_use'] + value['reserved']+value['allocated']
            ...
            for key, value in project_quotas.items():
                maximum = -1
                if(parent_id):
                    maximum = parent_project_quotas[key]['remains']-parent_project_quotas[key]['allocated'] +value['limit']
                 
                minimum = \
                    max(int(self._sub_quota_values(value['limit'],
                                                   value['remains'])),
                        int(value['in_use'] + value['reserved'] + value['allocated']))
                settable_quotas[key] = dict(minimum=minimum, maximum=maximum)
...
class QuotaEngine(object):
    ...
    def get_settable_quotas(self, context, project_id,parent_id=None,
                                              user_id=None):
        """Given a list of resources, retrieve the range of settable quotas for
        the given user or project.
